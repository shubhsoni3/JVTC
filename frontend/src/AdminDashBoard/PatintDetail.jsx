...
function SecurityAmount() {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = securityList.filter(item =>
      ...
      item.Patient_contact_number.includes(e.target.value) ||
      item.Patient_email.toLowerCase().includes(e.target.value.toLowerCase())
    );
  ...
  const getContactData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:4000/api/auth/PatientDetailGet"
      );
                    ...
                    <tbody>
                      {currentRows?.length === 0 ? (
                        ...
                        currentRows?.map((item, index) => (
                          <tr className="table-row" key={item.Patient_Detail_Id }>
                            <td>{item.Patient_name}</td>
                            <td>{item.Patient_email}</td>
                            <td>{item.Patient_contact_number}</td>
                            <td>{item.Patient_message}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
