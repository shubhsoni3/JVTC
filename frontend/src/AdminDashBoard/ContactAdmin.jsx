...
function SecurityAmount() {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = securityList.filter(item =>
      item.contact_Persion_name.toLowerCase().includes(e.target.value.toLowerCase()) ||
      item.contact_Persion_phone.includes(e.target.value) ||
      item.contact_Persion_email.toLowerCase().includes(e.target.value.toLowerCase())
    );
  ...
  const getContactData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:4000/api/auth/userdataget"
      );
      ...
      <div className="row flex-nowrap">
        ...
        <div className="col-lg-11 mt-2 set" id="set">
          <div className="text-center mt-4">
            ...
            <h3 className="fw-bold">Contact Us Data</h3>
          </div>
          <div className="row">
                      ...
                      {currentRows?.length === 0 ? (
                        ...
                        currentRows?.map((item, index) => (
                          <tr className="table-row" key={item.contact_id}>
                            <td>{item.contact_Persion_name}</td>
                            <td>{item.contact_Persion_address}</td>
                            <td>{item.contact_Persion_email}</td>
                            <td>{item.contact_Persion_phone}</td>
                            <td>{item.contact_Persion_subject}</td>
                            <td>{item.contact_Persion_message}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
